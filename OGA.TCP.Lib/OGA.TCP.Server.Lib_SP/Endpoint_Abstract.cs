...
#pragma warning restore CS8604 // Possible null reference argument.
        ...
        private async void ProcessChunkingMessage(string msgId, string messagetype, string data, string channel, string scope)
        {
            ...
            if(messagetype == nameof(ChunkStartDTO).ToLower())
            {
                ...
                this._largemsgreceivers.Add(msgId, lmr);
            ...
            else if(messagetype == nameof(ChunkDTO).ToLower())
            {
                ...
                if(this._largemsgreceivers.ContainsKey(msgId))
                {
                    // Have a receiver for the in-progress message.
                    var rcv = this._largemsgreceivers[msgId];
                ...
                else
                {
                    ...
                    OGA.SharedKernel.Logging_Base.Logger_Ref?.Error(
                        $"{_classname}:{this.InstanceId.ToString()}::{nameof(ProcessChunkingMessage)} - " +
                        $"Received message chunk without a chunk start message. Cannot process message, msgId: ({(msgId ?? "")}).");
            ...
            else if(messagetype == nameof(ChunkEndDTO).ToLower())
            {
                ...
                if(this._largemsgreceivers.ContainsKey(msgId))
                {
                    ...
                    // Remove the receiver...
                    this._largemsgreceivers.Remove(msgId);
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
                ...
                else
                {
                    ...
                    OGA.SharedKernel.Logging_Base.Logger_Ref?.Error(
                        $"{_classname}:{this.InstanceId.ToString()}::{nameof(ProcessChunkingMessage)} - " +
                        $"Received chunk end message without a chunk start message. Cannot process message, msgId: ({(msgId ?? "")}).");
